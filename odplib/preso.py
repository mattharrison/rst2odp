...
class Preso(object):
    ...
    def get_props(self, class_name, master_page_name=None):
        ...
        for t in self.template_files:
            ...
            return self.default_template.get_frame_properties(
                master_page_name, class_name
            )
    ...
    def import_slide(self, preso_file, page_num):
        ...
        slides = content_tree.findall(
            "{urn:oasis:names:tc:opendocument:xmlns:office:1.0}body/{urn:oasis:names:tc:opendocument:xmlns:office:1.0}presentation/{urn:oasis:names:tc:opendocument:xmlns:drawing:1.0}page"
        )
        try:
            slide_xml = slides[page_num - 1]
        except IndexError as e:
            sys.stderr.write(
                "Can't find page_num {} only {} slides".format(page_num, len(slides))
            )
            raise
...
"""
    ...
    def override_styles(self, data):
        if NORMAL_FONT != "Arial":
            data = data.replace(
                u'fo:font-family="Arial"', u'fo:font-family="%s"' % NORMAL_FONT
            )
        return data
    ...
    def get_xywh(self, measurement=None, slide=None):
        if slide and slide.grid_w_h_x_y:
            frame_w, frame_h, frame_x, frame_y = [
                int(float(x.replace("cm", ""))) for x in slide.grid_w_h_x_y
            ]
        else:
            frame_x = 0
            frame_y = 0
        ...
        if "crop" in classes:
            x, y, w, h = imagescale.adjust_crop(
                frame_w * DPCM, frame_h * DPCM, self.w, self.h
            )
...
class Slide(object):
    ...
    def _init_from_master_page(self, master_page_name):
        ...
        if master is None:
            sys.stderr.write(
                "MASTER NOT FOUND! {} Names: {}".format(
                    master_page_name, list(self.preso.get_master_page_names())
                )
            )
            self._init_xml()
            return
...
class XMLSlide(Slide):
    ...
    def page_num(self):
        ...
        name = self.page_node.attrib.get(
            "{urn:oasis:names:tc:opendocument:xmlns:drawing:1.0}name", None
        )
        return name
...
class TextFrame(MixedContent):
    ...
    def __init__(self, slide, attrib=None, props=None, style_name=None):
        ...
        attrib = attrib or {
            "presentation:style-name": style_name
            or props.get("style-name", "Default-outline1"),
...
class TextStyle(object):
    ...
    text_underline_type = dict(NONE="none", SINGLE="single", DOUBLE="double")  # default
    ...
    text_underline_mode = dict(SKIP_WHITE_SPACE="skip-white-space")
    font_variant = dict(NORMAL="normal", SMALL_CAPS="small-caps")
    text_transform = dict(
        NONE="none",
        LOWERCASE="lowercase",
        UPPERCASE="uppercase",
    ...
    text_outline = dict(TRUE="true")
    text_rotation_angle = dict(ZERO="0", NINETY="90", TWOSEVENTY="270")
    text_rotation_scale = dict(LINE_HEIGHT="line-height", FIXED="fixed")
    ...
    def __repr__(self):
        return "({} name:{} styles:{})".format(self.__class__, self.name, self.styles)
...
class ParagraphStyle(TextStyle):
    text_align = dict(START="start", END="end", CENTER="center", JUSTIFY="justify")
    margin_left = None
    margin_right = None
    margin_top = None
    margin_bottom = None
...
http://books.evc-cit.info/odbook/ch03.html#bulleted-numbered-lists-section
 ...
 <draw:text-box>
    ...
    def __init__(self, slide, attrib=None):
        ...
        self.slide.pending_styles.append(
            ParagraphStyle(**{"text:enable-numbering": "true"})
        )
...
class TableFrame(MixedContent):
    ...
    def __init__(self, slide, frame_attrib=None, table_attrib=None):
        ...
        self.attrib = table_attrib or {
            "table:template-name": "default",
            "table:use-first-row-styles": "true",
            "table:use-banding-rows-styles": "true",
        }
...
class Template(object):
    ...
    def _get_frame_properties(self, style_name, class_name, sub_elem, debug=False):
        ...
        for elem in self.styles.findall(xpath):
            res = dict((key.split("}")[-1], value) for key, value in elem.items())
            return res

        master_names = list(self.get_master_page_names())
        if style_name not in master_names:
            raise KeyError(
                "Master page name '{}' not in template. Available names:{}".format(
                    style_name, master_names
                )
            )
    ...
    def get_span_properties(self, style_name, class_name):
        ...
        span_dict = self._get_frame_properties(
            style_name, class_name, ns("text", "span")
        )
        if not span_dict:
            return
